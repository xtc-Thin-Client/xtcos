...
DATA_PART_START=$((BOOT_PART_START + BOOT_PART_SIZE))
DATA_PART_SIZE=$(((DATA_SIZE + ALIGN - 1) / ALIGN * ALIGN))
ROOT_PART_START=$((BOOT_PART_START + BOOT_PART_SIZE + DATA_PART_SIZE ))
ROOT_PART_SIZE=$(((ROOT_SIZE + ROOT_MARGIN + ALIGN  - 1) / ALIGN * ALIGN))


IMG_SIZE=$((BOOT_PART_START + BOOT_PART_SIZE + DATA_PART_SIZE + ROOT_PART_SIZE))
...
DATA_OFFSET=$(echo "$PARTED_OUT" | grep -e '^2:' | cut -d':' -f 2 | tr -d B)
DATA_LENGTH=$(echo "$PARTED_OUT" | grep -e '^2:' | cut -d':' -f 4 | tr -d B)
